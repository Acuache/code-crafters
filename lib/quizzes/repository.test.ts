...
describe("quiz repository", () => {
  it("removes answers and explanations from the client DTO", () => {
    expect(toSafeQuiz(ready).questions).toEqual([
      { id: "1", prompt: "P", options: ["A", "B", "C", "D"] },
    ]);
  });

  it("reuses an existing ready quiz without generating", async () => {
    const store: QuizStore = {
      findActive: async () => ready,
      claim: async () => {
    ...
    let generated = false;
    const result = await getOrCreateQuiz(
      { targetKey: "course:1", title: "Curso", kind: "course" },
      {
        store,
        generate: async () => {
          generated = true;
  ...
  it("publishes only the winner of a shared claim", async () => {
    ...
    const store: QuizStore = {
      ...
      claim: async () =>
        record
          ...
          : {
              record: (record = { ...ready, status: "generating", questions: null }),
              claimed: true,
            },
    ...
    const [first, second] = await Promise.all([
      getOrCreateQuiz(
        { targetKey: "course:1", title: "Curso", kind: "course" },
        { store, generate, wait: async () => undefined },
      ),
      getOrCreateQuiz(
        { targetKey: "course:1", title: "Curso", kind: "course" },
        { store, generate, wait: async () => undefined },
