...
export function CourseDetail({ step, onCourseQuiz, onChapterQuiz }: {
  step: PathStepView;
  onCourseQuiz(): void;
  onChapterQuiz(chapter: string): void;
}) {
  ...
  return <Card><CardHeader><CardTitle><h2>{step.course.title}</h2></CardTitle></CardHeader><CardContent>
    ...
    <Separator />
    <ul className="space-y-2">{step.course.chapters.map((chapter) => <li className="flex items-center justify-between gap-2" key={chapter}><span>{chapter}</span><Button onClick={() => onChapterQuiz(chapter)} size="sm" variant="outline">Practicar</Button></li>)}</ul>
    <Button disabled={locked} onClick={onCourseQuiz}>{locked ? <LockIcon /> : null}{locked ? "Completa el paso anterior" : "Comenzar quiz del curso"}</Button>
